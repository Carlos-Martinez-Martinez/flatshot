...
## Fase 57 - Extraccion de estado del encabezado de subvista del inspector

Estado: completada.

Cambios realizados:
- `inspector-context-view.js` ahora expone `inspectorSubviewHeaderState()` para decidir titulo, subtitulo, detalle, accion de vuelta y accion de gestion.
- `app.js` conserva los datos de estado (`outputEditMode`, preset activo, preset editor, avisos y label de salida); solo delega la decision de encabezado.
- Se amplio `tests/test_frontend_inspector_context_view.py` para cubrir:
  - salida en edicion;
  - ajuste avanzado normal;
  - gestor de ajustes;
  - avisos con conteo plural;
  - fallback de modo desconocido.

Impacto medido:
- `app.js`: 6.512 -> 6.506 lineas.
- Modulos JS frontend: sin cambios, 23.
- Tests frontend `test_frontend_*.py`: sin cambios, 22.
...
## Fase 59 - Extraccion de onboarding inicial

Estado: completada.

Cambios realizados:
- `empty-state-view.js` ahora expone `initialStateHtml()` para renderizar el estado inicial de seleccion de carpeta.
- `app.js` conserva `devMode` y `bridgeScanPath`; solo delega el HTML de onboarding.
- Se movio fuera de `app.js`:
  - icono SVG del estado inicial;
  - acciones `pick-bridge-folder` y `open-app-settings`;
  - bloque dev de ruta manual;
  - escaping de la ruta manual.
- Se amplio `tests/test_frontend_empty_state_view.py` para cubrir onboarding normal y modo dev.

Impacto medido:
- `app.js`: 6.501 -> 6.478 lineas.
- Modulos JS frontend: sin cambios, 23.
- Tests frontend `test_frontend_*.py`: sin cambios, 22.

Validaciones ejecutadas:
- `node --check apps/flatshot-desktop/frontend/empty-state-view.js`: OK.
- `node --check apps/flatshot-desktop/frontend/app.js`: OK.
- `venv\Scripts\python.exe -m pytest tests/test_frontend_empty_state_view.py -q`: 2 passed.
- `venv\Scripts\python.exe -m pytest -q`: 282 passed.
- `Get-ChildItem apps/flatshot-desktop/frontend -Filter *.js | ForEach-Object { node --check $_.FullName }`: OK.
- `git diff --check`: OK; solo avisos Git LF/CRLF en Windows.
- HTTP local: `index.html`, `empty-state-view.js` y `app.js` respondieron 200.
...
## Fase 64 - Limpieza de funciones frontend muertas

Estado: completada.

Cambios realizados:
- Se retiraron de `app.js` funciones declaradas sin referencias activas:
  - `activePresets()`;
  - `outputPresetLabel()`;
  - `reviewErrors()`;
  - `sameOutputProfile()`.
- Se mantuvo intacto el handler real de `review-errors`, que ya delegaba directamente en `reviewWarnings()`.
- Se amplio `tests/test_frontend_app_cleanup.py` para evitar reintroducir esas funciones muertas.

Impacto medido:
- `app.js`: 6.417 -> 6.394 lineas.
- Modulos JS frontend: sin cambios, 23.
- Tests frontend `test_frontend_*.py`: sin cambios, 23.
...
- `venv\Scripts\python.exe -m pytest -q`: 283 passed.
- `Get-ChildItem apps/flatshot-desktop/frontend -Filter *.js | ForEach-Object { node --check $_.FullName }`: OK.
- HTTP local: `index.html`, `app.js`, `output-profile-view.js` y `gallery.js` respondieron 200.

Salida exportada:
- Sin cambios esperados. No se modifico `ExportRunner`, bridge Python, motor de imagen ni escritura de archivos.

## Fase 65 - Extraccion de estado del badge de lote

Estado: completada.

Cambios realizados:
- `batch-view.js` ahora expone:
  - `adjustedCountLabel()`;
  - `batchPillState()`.
- `app.js` conserva los conteos (`issueCount`, `adjusted`) y delega el label/tono del badge de lote.
- Se retiro `pluralizeCount()` de `app.js`, que ya solo se usaba para el label de imagen ajustada.
- Se ampliaron:
  - `tests/test_frontend_batch_view.py` para cubrir avisos, ajustadas y estado listo;
  - `tests/test_frontend_app_cleanup.py` para evitar reintroducir `pluralizeCount()`.

Impacto medido:
- `app.js`: 6.394 -> 6.383 lineas.
- Modulos JS frontend: sin cambios, 23.
- Tests frontend `test_frontend_*.py`: sin cambios, 23.
...
## Fase 67 - Limpieza de wrappers de labels sin uso
...
- Los helpers reales siguen disponibles y probados en `batch-view.js`.
- Se amplio `tests/test_frontend_app_cleanup.py` para evitar reintroducir estos wrappers.

Impacto medido:
- `app.js`: 6.374 -> 6.362 lineas.
- Modulos JS frontend: sin cambios, 23.
- Tests frontend `test_frontend_*.py`: sin cambios, 23.
...
## Fase 69 - Limpieza de wrappers de presentacion

Estado: completada.

Cambios realizados:
- Se retiraron de `app.js` wrappers delgados o sin uso:
  - `folderItemHtml()`;
  - `progressPanelHtml()`;
  - `outputTemporaryNoticeHtml()`;
  - `issueItemHtml()`;
  - `preflightListHtml()`.
- Los puntos de uso ahora llaman directamente a los helpers de `batch-detail-view.js`, `export-preflight-view.js` e `inspector-output-view.js`.
- Se amplio `tests/test_frontend_app_cleanup.py` para evitar reintroducir estos wrappers.

Impacto medido:
- `app.js`: 6.353 -> 6.338 lineas.
- Modulos JS frontend: sin cambios, 23.
- Tests frontend `test_frontend_*.py`: sin cambios, 23.

Validaciones ejecutadas:
- `node --check apps/flatshot-desktop/frontend/app.js`: OK.
- `venv\Scripts\python.exe -m pytest tests/test_frontend_app_cleanup.py tests/test_frontend_batch_detail_view.py tests/test_frontend_export_preflight_view.py tests/test_frontend_inspector_output_view.py -q`: 7 passed.
- `venv\Scripts\python.exe -m pytest -q`: 283 passed.
- `Get-ChildItem apps/flatshot-desktop/frontend -Filter *.js | ForEach-Object { node --check $_.FullName }`: OK.
- HTTP local: `index.html`, `app.js`, `batch-detail-view.js`, `export-preflight-view.js` e `inspector-output-view.js` respondieron 200.
