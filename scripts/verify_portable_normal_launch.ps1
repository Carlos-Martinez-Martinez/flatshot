...
function Get-UiAutomationEvidence {
    param([long]$Handle)
    $root = [System.Windows.Automation.AutomationElement]::FromHandle([IntPtr]$Handle)
    if ($null -eq $root) {
        throw "UI Automation could not inspect FlatShot handle $Handle."
    }
    ...
    $expectedLabels = @("FlatShot", "Lote", "Exportación", "Destino")
    $matchedLabels = @($expectedLabels | Where-Object {
        $expected = $_
...
try {
    ...
    if ($null -ne $window) {
        ...
        $screenshot = Save-WindowScreenshot -Handle $window.Handle -Path $ScreenshotPath -RenderTimeoutSeconds 2
        $screenshot | Add-Member -NotePropertyName captureLimitation -NotePropertyValue (
